...
export class DocumentService extends BaseService {
  ...
  async createDocument(data: CreateDocumentInput, authorId: string) {
    try {
      ...
      // Previously enqueued as a BullMQ 'doc-created' job whose worker wrote this
      // row. Written directly now. Note it notifies the author about their own
      // document — the original carried a 'send to all workspace members' TODO
      // that was never implemented; behaviour is preserved as-is.
  ...
  async updateDocument(id: string, data: UpdateDocumentInput, userId: string) {
    try {
      ...
      // readingTime is a real column, so build a typed patch rather than
      // widening updateData. The audit entry below records Object.keys of this
      // patch, which therefore includes readingTime whenever content changed
      // and excludes changeSummary — matching the previous behaviour exactly.
